...
const CruiseDetail = () => {
  ...
  const fetchReviews = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_APP_URL_FE_PUBLIC}/review/accommodation/${id}`,
      );
...
Với các tiện nghi cao cấp như ${
      hotel.amenities || "hồ bơi, spa, nhà hàng và wifi miễn phí"
    }, 
${hotel.name} hứa hẹn mang lại trải nghiệm nghỉ dưỡng hoàn hảo cho du khách.

Du khách có thể dễ dàng di chuyển đến các địa điểm du lịch nổi tiếng trong khu vực. 
Đội ngũ nhân viên chuyên nghiệp luôn sẵn sàng phục vụ 24/7 nhằm mang đến sự hài lòng tối đa.

${hotel.name} là lựa chọn lý tưởng cho kỳ nghỉ gia đình, cặp đôi hoặc chuyến công tác.
`;
  };
  ...
  return (
    <div className="hotel-detail-page">
      ...
      <section id="description" className="hotel-section-s">
        ...
        <img src={hotel.imageUrls?.[2]} className="desc-img" />

        <p>{generateHotelDescription(hotel)}</p>

        <img src={hotel.imageUrls?.[3]} className="desc-img" />
      </section>
